...
const isAdmin = loggedInUser?.role === "admin";
  ...
  const fetchUserDetails = useCallback(async () => {
    try {
      ...
      const [detailRes, stateRes] = await Promise.all([
        fetch(`${API_BASE_URL}/address/${id}`, { headers: authHeaders() }),
  ...
  const fetchPendingEditStatus = useCallback(async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/pending/edit-status`, {
        headers: authHeaders()
      });
            ...
            <div style={fieldGroupStyle}>
              ...
              {typeof user.profile_image === 'string' && user.profile_image && (
                <div style={{ marginTop: '5px' }}>
                  ...
                  <a 
                    href={user.profile_image.startsWith('http') ? user.profile_image : `${API_BASE_URL}/${user.profile_image.replace(/\\/g, '/')}`} 
                    target="_blank" 
                    rel="noopener noreferrer"
                  >
                    <img 
                      src={user.profile_image.startsWith('http') ? user.profile_image : `${API_BASE_URL}/${user.profile_image.replace(/\\/g, '/')}`} 
                      alt="Profile" 
                    ...
                    <div style={{ marginTop: '5px' }}>
                      {typeof address.address_image === "string" && address.address_image && (
                        <div style={{ marginTop: "5px" }}>
                          ...
                          <a
                            href={address.address_image.startsWith("http") ? address.address_image : `${API_BASE_URL}/${address.address_image.replace(/\\/g, "/")}`}
                            target="_blank"
                            rel="noopener noreferrer"
                          >
                            <img
                              src={address.address_image.startsWith("http") ? address.address_image : `${API_BASE_URL}/${address.address_image.replace(/\\/g, "/")}`}
                              alt="Address Doc"
              ...
              <div key={employment.employment_id || index} style={blockStyle}>
                ...
                <div style={{ background: "#f0f4f8", padding: "12px", borderRadius: "6px", marginBottom: "15px" }}>
                  ...
                  {(employment.salaries || []).map((salaryRow, salaryIndex) => {
                    ...
                    const fileUrl = typeof salaryRow.salary_image === 'string' && (salaryRow.salary_image.startsWith('http') ? salaryRow.salary_image : `${API_BASE_URL}/${salaryRow.salary_image}`);
...
{!isAdmin && (hasPendingRequest || user.user_status === "9") && (
  <div
    style={{
...
<div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "20px" }}>
  ...
  <button
    type="submit"
    style={buttonStyle}
    disabled={submitting || hasPendingRequest || (!isAdmin && user.user_status === "9")}
  >
