...
function PendingApprovals() {
  ...
  const fetchPendingRequests = useCallback(async () => {
    try {
      setLoading(true);
      const res = await fetch(apiRoutes.pending, { headers: authHeaders() });
  ...
  const handleReviewClick = async (req) => {
    ...
    if (req.created_by) {
      try {
        ...
        const res = await fetch(apiRoutes.userDetails(req.created_by), {
          headers: authHeaders() 
        });
  ...
  const handleApprove = async (id) => {
    try {
      ...
      const res = await fetch(apiRoutes.approvePending(id), {
        method: "PUT",
        headers: authHeaders()
      });
      const result = await res.json();
  ...
  const handleReject = async (id) => {
    try {
      setProcessing(true);
      setMessage({ type: "", text: "" });
      const res = await fetch(apiRoutes.rejectPending(id), {
        method: "PUT",
        headers: authHeaders()
      });
      const result = await res.json();
              ...
              {fetchingCurrentData ? (
                ...
                <>
                  ...
                  <div style={{ display: "flex", gap: "15px", marginTop: "30px", borderTop: "1px solid #ddd", paddingTop: "20px" }}>
                    ...
                    <button onClick={() => handleApprove(selectedRequest.id)} style={{ flex: 1, padding: "12px", backgroundColor: "#28a745", color: "white", border: "none", borderRadius: "6px", cursor: "pointer", fontWeight: "bold" }} disabled={processing}>{processing ? "Processing..." : "Approve"}</button>
                  </div>
                </>
              )}
            </div>
